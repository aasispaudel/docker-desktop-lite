...
log "Docklite macOS Colima runtime reset"
confirm "${1:-}"
...
if has colima; then log "Stopping Colima"; colima stop >/dev/null 2>&1 || true; log "Deleting Colima VM/data"; colima delete --force >/dev/null 2>&1 || warn "Colima may already be deleted."; else warn "Colima not installed."; fi
